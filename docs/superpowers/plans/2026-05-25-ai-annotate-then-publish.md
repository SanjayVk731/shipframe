...
> **CORRECTION (2026-05-25, during execution):** The spec and this plan were drafted against a four-AI-provider assumption (Anthropic, OpenAI, Azure OpenAI, **Ollama**) with a per-provider `model` field. The actual code merged to `main` ships **three** providers only — `AiProvider = 'anthropic' | 'openai' | 'azure-openai'` — with an `endpoint` (Azure) but **no Ollama adapter and no `model` field**. `src/ui/ai/ollama.ts` does not exist; `draft.ts` does not import `callOllama`; `AiConfig` has no `model`. Accordingly, **Tasks 7, 8, and 12 are scoped to the three real providers. All Ollama and `model` references in the steps below are dropped.** This keeps the plan aligned with the codebase; the vision feature is provider-complete with the three that exist.

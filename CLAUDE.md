...
- **Notion**: Property names are discovered case-insensitively from the database schema (`discoverPropertyNames`) — "Type", "type", and "TYPE" are all matched. The Figma deep link goes in a callout block. When a `TicketInput.inlineImage` is present, Notion uploads it via the `file_uploads` API (two-step create → send, `uploadFileForBlock`) and appends an image block; the standalone `uploadAttachment` is still a no-op (`supported: false`). Notion **ignores** `acceptanceCriteriaHtml` and `reproStepsHtml` today — those fields are Azure-only.
...
2. `sanitizeProse()` (from `src/ui/sanitize.ts`) runs `DOMPurify.sanitize()` with an `ALLOWED_TAGS` whitelist (`p`, `br`, `strong`, `em`, `b`, `i`, `code`, `pre`, `a`, `ul`, `ol`, `li`, `h2`, `h3`, `blockquote`) and `ALLOWED_ATTR: ['href', 'src', 'alt']`. `FORBID_TAGS` blocks `script`, `style`, `iframe`, and **`img` is NOT in the prose tag list** — user-typed/AI-drafted images are stripped (an attacker-controlled `<img src>` in a ticket body is a tracking-beacon / SSRF-on-viewer primitive). Event-handler attrs and `javascript:` URLs are stripped too.
...
**Sanitization lives in `src/ui/sanitize.ts`** (two tiers): `sanitizeProse` (no `<img>`, used by the composer) and `sanitizeHtml` (allows `<img>`, used ONLY by the Azure provider to wrap a frame screenshot it just uploaded — the src points at the provider's own attachment store, not an arbitrary host). The composer imports `sanitizeProse`; the Azure provider imports `sanitizeHtml` from `sanitize.ts` (not from the composer — keep providers off the composer's import graph).
...
Pin lifecycle is managed entirely in the sandbox via `figma.annotations`. `syncAnnotation` is idempotent — it reconciles to a single canonical pin per node and ticket: noop when an ours-pin already exists, write when there is none, preserve any non-ours annotation. Pin labels follow `AZURE-<id>` or `Notion · <title> · #<8-hex>`; AI-draft pins carry the ticket id as a trailing `\n— AZURE-<id>` / `\n— Notion #<8-hex>` suffix (see `appendTicketIdToAnnotation`, which is idempotent — it won't double-append a suffix the body already ends with). `isOursTail`'s regex (`OURS_TAIL_RE`, matching either the bare short label or that trailing suffix) is the **single source of truth** for ownership — never parse pin labels by hand elsewhere. The Notion short-id derivation is centralized in `notionShortId()`; truncation-with-ellipsis is `truncateWithEllipsis()` in `src/shared/text.ts`. `clearAnnotation` removes only ours-pins; it's wired in the message protocol but not currently surfaced in any UI action (kept around for a future "delete pin" flow). The sandbox handler in `main.ts` translates `sync-annotation` / `clear-annotation` failures to the generic `error` envelope.
...
BYO-key, **vision-capable**. Three providers: `anthropic` (`api.anthropic.com`), `openai` (`api.openai.com`), and `azure-openai` (a user-supplied `*.openai.azure.com` endpoint, validated by `isAzureOpenAiEndpoint` in `aiConfig.ts`). The UI iframe calls the chosen API directly using the user's key from `clientStorage` (`ai:provider` + `ai:key` + optional `ai:endpoint`, atomic clear via `clearAiConfig`). The sandbox only collects context via `get-frame-context` + exports the thumbnail; **no network in the sandbox**.

By default a draft also sends a downscaled screenshot of the frame (`downscaleForVision`, longest edge `MAX_VISION_EDGE` = 1024px) — the per-draft "Include image" toggle in CreateView turns this off for a text-only draft. The Settings privacy copy must stay accurate to this (it discloses the screenshot). Reuses providers' `Result<T>` and `NormalizedReason` — no parallel error-shape system. `parseDraftResponse` strips ```` ```json ```` fences, validates JSON shape, filters keys by WIT (same mapping as `sectionSetFor` in CreateView), coerces numbers/booleans to strings, and truncates `pinMarkdown` (`truncateWithEllipsis`). The orchestrator (`draft.ts`) retries once on malformed JSON with a stricter reminder; if still malformed, returns `reason: 'unknown'`. Anthropic adapter sends `anthropic-dangerous-direct-browser-access: true` (required for browser-direct calls). OpenAI adapter sets `response_format: { type: 'json_object' }`. `draft.ts` is **dynamic-imported** from CreateView so the AI bundle is only fetched when the user clicks Draft.
...
- **Don't widen `networkAccess.allowedDomains`** without updating both `manifest.json` and `SECURITY.md` — Figma reviewers read both. The shipped manifest is **Azure-only**: locked to `dev.azure.com`, `*.visualstudio.com`, `api.anthropic.com`, `api.openai.com`, `*.openai.azure.com`. The Notion provider is fully implemented in `src/providers/notion.ts` but `api.notion.com` is **not** in the allowlist — re-add it to the manifest (and re-expose Notion in the SettingsView picker) before shipping Notion.
